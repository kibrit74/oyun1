...
async def main(product_name):
    async with async_playwright() as p:
        try:
            ...
            json_filename = f'{sanitized_product_name}_data.json'
...
def compare_content():
    try:
        ...
        product_name1 = response1.text.strip()
        product_name2 = response2.text.strip()

        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        if product_name_input1:
            json_filename1 = loop.run_until_complete(main(product_name_input1))
        else:
            json_filename1 = loop.run_until_complete(main(product_name1))

        if product_name_input2:
            json_filename2 = loop.run_until_complete(main(product_name_input2))
        else:
            json_filename2 = loop.run_until_complete(main(product_name2))
        ...
        prompt_parts = [
            f"adı veya koduyla eşleştir ve kısa bir şekilde tanımla, Öncelikle ürünlerin teknik özelleklerini getir. olumlu ve olumsuz yönlerini içeren detaylı bir analiz yap sonuç kısmında oldukça detaylı bir genel bir değerlendirme yap, Sonuç kısmında iki ürünü detalı olarak karşılaştır ve kıyaslama yapacaksın.",
            ...
            " ",
            {"text": json.dumps({'product1': data1, 'product2': data2}, ensure_ascii=False, indent=4)},
        ]
...
def generate_content():
    try:
        ...
        if product_name_input:
            json_filename = loop.run_until_complete(main(product_name_input))
        else:
            json_filename = loop.run_until_complete(main(product_name))
        ...
        # Send the data to the model for analysis
        prompt_parts = [
            f"Kullanıcının gönderdiği fotoğraftaki ürünü tanımlıyorum: Öcelikle ürününün Teknik özelliklerini vericeğim, Olumlu ve olumsuz yorum özetleri veriyorum . Olumlu ve olumsuz yönlerini içeren detaylı bir analiz yap sonuç kısmında oldukça detaylı bir genel bir değerlendirme yap,",
